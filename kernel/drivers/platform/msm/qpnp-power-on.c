...
// khs : Powerkey delay(sleep/wakeup delay).
#define USE_QPNP_POWERKEY_STATUS_DELAY
#endif
...
struct qpnp_pon_config {
	...
	bool old_state;
	bool use_bark;
#if defined(USE_QPNP_POWERKEY_STATUS_DELAY)
	u32 powerkey_delay;
#endif
};
...
struct qpnp_pon {
	...
	bool kpdpwr_work_status;
	bool kpdpwr_press_status;
#endif
...
#if defined(USE_QPNP_POWERKEY_STATUS_DELAY)
// haksukim
static void kpdpwr_work_func(struct work_struct *work)
{
	...
	key_status = pon_rt_sts & QPNP_PON_KPDPWR_N_SET;

    if (pon->kpdpwr_press_status) {
...
qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	 ...
	 */
#if defined(USE_QPNP_POWERKEY_STATUS_DELAY)
	if (!cfg->old_state && !key_status && cfg->pon_type != PON_KPDPWR) {
#else
	...
	if (cfg->pon_type == PON_KPDPWR) {
		...
		if (key_status == 1) {
			pon->kpdpwr_work_status = true;
			pon->kpdpwr_press_status = true;
			disable_irq_nosync(cfg->state_irq);
			schedule_delayed_work(&pon->kpdpwr_work, msecs_to_jiffies(cfg->powerkey_delay));
		} else {
			pon->kpdpwr_press_status = false;
			if (pon->kpdpwr_work_status) {
...
#if defined(USE_QPNP_POWERKEY_STATUS_DELAY)
	// haksukim
	INIT_DELAYED_WORK(&pon->kpdpwr_work, kpdpwr_work_func);
#endif
...
#if defined(USE_QPNP_POWERKEY_STATUS_DELAY)
	rc = of_property_read_u32(pon->spmi->dev.of_node, "markt,powerkey-delay", &pon->pon_cfg->powerkey_delay);
	if (rc) {
		dev_err(&pon->spmi->dev, "markt,powerkey-delay default 0 ms\n");
		pon->pon_cfg->powerkey_delay = 300;
	}
#endif
