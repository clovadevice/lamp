...
static void max17048_work(struct work_struct *work)
{
	...
	if (voltage_diff >= 50 || chip->capacity_level != chip->lasttime_capacity_level || temp_diff >= 5) {
		...
		if (get_usb_status(chip) == false) {
			if (chip->lasttime_capacity_level < chip->capacity_level) {
		...
		struct max17048_chip *chip)
{
	...
	if(ret == 0){
		if (strict_strtoul(battery_type, 0, &value)){
			pr_err("Invalid battery_type :%s\n", battery_type);
		}

		ret = of_property_read_u32(dev_node, "max17048,max-battery-type",
				&max_battery_typ);
		if (ret) {
	...
	strcpy(max17048_dts, "max17048,rcomp_");
	strcat(max17048_dts, battery_type);
	ret = of_property_read_u32(dev_node, max17048_dts,
				&chip->rcomp);
	if (ret) {
	...
	chip->rcompseg = 0;
	strcpy(max17048_dts, "max17048,rcompseg_");
	strcat(max17048_dts, battery_type);
	ret = of_property_read_u32(dev_node, max17048_dts,
				&chip->rcompseg);
	if (ret) {
	...
	strcpy(max17048_dts, "max17048,max-mvolt_");
	strcat(max17048_dts, battery_type);
	ret = of_property_read_u32(dev_node, max17048_dts,
				   &chip->max_mvolt);
	if (ret) {
	...
	strcpy(max17048_dts, "max17048,min-mvolt_");
	strcat(max17048_dts, battery_type);
	ret = of_property_read_u32(dev_node, max17048_dts,
				   &chip->min_mvolt);
	if (ret) {
	...
	strcpy(max17048_dts, "max17048,fcc-mah_");
	strcat(max17048_dts, battery_type);
	ret = of_property_read_u32(dev_node, max17048_dts,
				   &chip->fcc_mah);
	if (ret) {
	...
	strcpy(max17048_dts, "max17048,soc-check-a_");
	strcat(max17048_dts, battery_type);
	ret = of_property_read_u32(dev_node, max17048_dts,
	...
	strcpy(max17048_dts, "max17048,soc-check-b_");
	strcat(max17048_dts, battery_type);
	ret = of_property_read_u32(dev_node, max17048_dts,
	...
	strcpy(max17048_dts, "max17048,ocv-test_");
	strcat(max17048_dts, battery_type);
	ret = of_property_read_u32(dev_node, max17048_dts,
				   &chip->ocv_test);
	if (ret) {
	...
	strcpy(max17048_dts, "max17048,custom-model-data_");
	strcat(max17048_dts, battery_type);
	if (!of_find_property(dev_node, max17048_dts, &prop_len)) {
	...
	ret = of_property_read_u8_array(dev_node, max17048_dts,
	...
	strcpy(max17048_dts, "max17048,rcomp_seg_");
	strcat(max17048_dts, battery_type);
	if (!of_find_property(dev_node, max17048_dts, &prop_len)) {
	...
	strcpy(max17048_dts, "max17048,bits_");
	strcat(max17048_dts, battery_type);
	ret = of_property_read_u32(dev_node, max17048_dts,
				   &chip->bits);
	if (ret) {
		pr_err("%s: failed to read bits\n", __func__);
		goto out;
	}
