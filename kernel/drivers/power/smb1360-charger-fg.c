...
static int smb_parse_dt(struct smb1360_chip *chip)
{
	...
	if(ret == 0){
		...
		ret = of_property_read_u32(node, "max17048,max-battery-type",
				&max_battery_typ);
		if (ret) {
...
#if defined(USE_BATTERY_TYPE)
	strcpy(max17048_dts, "qcom,float-voltage-mv_");
	strcat(max17048_dts, battery_type);
	rc = of_property_read_u32(node, max17048_dts,
						&chip->vfloat_mv);
#else
...
#if defined(USE_BATTERY_TYPE)
	strcpy(max17048_dts, "qcom,iterm-ma_");
	strcat(max17048_dts, battery_type);
	rc = of_property_read_u32(node, max17048_dts, &chip->iterm_ma);
#else
...
#if defined(USE_BATTERY_TYPE)
	strcpy(max17048_dts, "qcom,fg-iterm-ma_");
	strcat(max17048_dts, battery_type);
	rc = of_property_read_u32(node, max17048_dts,
					&chip->fg_iterm_ma);
#else
