...
#define USE_MODEL_BOOT_LED_COLOR
...
#elif defined(CONFIG_TARGET_PRODUCT_IF_S1200N)
#include "anim_fade_in_out_lamp.c"
static struct ledanim_t ledanim_boot = {
	.name = "__boot__",
	.interval = 40,
	.anim_count = 1584,
...
{
#ifdef USE_MODEL_BOOT_LED_COLOR
	/* 
		CLOVADEVICEKIT2-2154
		NL-S200KR/JP : 105, 170, 60  : 0x69AA3C
		NL-S210KR/JP : 81, 75, 24	 : 0x514B18
		NL-S220KR/JP : 116, 170, 60	 : 0x74AA3C
		NL-S230KR/JP : 127, 82, 23	 : 0x7F5217

		update(upper rev0.5)
		NL-S200## : 174, 180, 28	 : 0xaeb41c
		NL-S210## : 81, 75, 18		 : 0x514b12
		NL-S220## : 81, 90, 19		 : 0x515a13
		NL-S230## : 81, 90, 16		 : 0x515a10
	*/
	struct _bootanim_desc {
		const char * modelname[3];
		unsigned char * anim;
		unsigned long color;
	};
	struct _bootanim_desc _bootanim_tbl_old[] = {
		{
		...
		{
			.modelname  = { "NL-S200KR", "NL-S200JP", "NL-S200KRL" },
			.anim		= anim_fade_in_out_69aa3c,
			.color		= 0x69AA3C,
		},
		{
			.modelname  = { "NL-S210KR", "NL-S210JP", "NL-S210KRL" },
			.anim		= anim_fade_in_out_514b18,
			.color		= 0x514b18,
		},
		{
			.modelname  = { "NL-S220KR", "NL-S220JP", "NL-S220KRL" },
			.anim		= anim_fade_in_out_74aa3c,
			.color		= 0x74aa3c,
		},
		{
			.modelname  = { "NL-S230KR", "NL-S230JP", NULL },
			.anim		= anim_fade_in_out_7f5217,
			.color		= 0x7f5217,
		},
	};
	struct _bootanim_desc _bootanim_tbl_def[] = { 
		{
			.modelname  = { NULL, NULL, NULL },
			.anim		= anim_fade_in_out,
			.color		= 0xff9632,
		...
		{
			.modelname  = { "NL-S200KR", "NL-S200JP", "NL-S200KRL" },
			.anim		= anim_fade_in_out_aeb41c,
			.color		= 0xaeb41c,
		},
		{
			.modelname  = { "NL-S210KR", "NL-S210JP", "NL-S210KRL" },
			.anim		= anim_fade_in_out_514b12,
			.color		= 0x514b12,
		},
		{
			.modelname  = { "NL-S220KR", "NL-S220JP", "NL-S220KRL" },
			.anim		= anim_fade_in_out_515a13,
			.color		= 0x515a13,
		},
		{
			.modelname  = { "NL-S230KR", "NL-S230JP", NULL },
			.anim		= anim_fade_in_out_515a10,
			.color		= 0x515a10,
		},
		{
			.modelname  = { "NL-S240KR", "NL-S240JP", NULL },
			.anim		= anim_fade_in_out_a7520d,
			.color		= 0xa7520d,
		},
		{
			.modelname  = { "NL-S250KR", "NL-S250JP", NULL },
			.anim		= anim_fade_in_out_7f5c17,
			.color		= 0x7f5c17,
		},
		{
			.modelname  = { "NL-S260KR", "NL-S260JP", NULL },
			.anim		= anim_fade_in_out_936617,
			.color		= 0x936617,
		},
		{
			.modelname  = { "NL-S270KR", "NL-S270JP", NULL },
			.anim		= anim_fade_in_out_a75c17,
			.color		= 0xa75c17,
		},
	};
	...
	char modelname[64] = {0,};
	int selected=0;
	struct _bootanim_desc * _bootanim_tbl;	
	int count;
	unsigned char rev;

	// default
	_bootanim_tbl = _bootanim_tbl_def;
	count = sizeof(_bootanim_tbl_def)/sizeof(_bootanim_tbl_def[0]);

	if(get_clova_board_rev(&rev) == 0 && rev < 0x05 ){
		// for old version
		_bootanim_tbl = _bootanim_tbl_old;
		count = sizeof(_bootanim_tbl_old)/sizeof(_bootanim_tbl_old[0]);
	}

	if(get_clova_modelname(modelname, sizeof(modelname) - 1) != 0){
		selected = 0;
		printk(KERN_ERR "get_clova_modelname() failed - use default led color\n");
	} else {
		int i,j;
		printk("modelname: %s\n", modelname);
		...
		for(i = 1; i < count; i++){
			for(j = 0; j < 3; j++){
				if(_bootanim_tbl[i].modelname[j] && 
						strcmp(_bootanim_tbl[i].modelname[j], modelname) == 0){
					selected = i;
					break;
				}
			}
	...
	printk("start boot animation. name:fade_in_out, color:0x%06X\n", (unsigned int)_bootanim_tbl[selected].color);
	ledanim_boot.anim = _bootanim_tbl[selected].anim;
