...
#define BTSCO_RATE_16KHZ 16000
	...
	#define MI2S_DATAOUT_PORT AFE_PORT_I2S_SD1
#else
	#define MI2S_DATAOUT_PORT AFE_PORT_I2S_SD0
#endif
...
static int msm_mi2s_tx_ch = 2;
	...
	defined(CONFIG_TARGET_PRODUCT_IF_S1200N)
...
#if defined(CONFIG_SND_SOC_CX2092X)
	#define USE_QUAT_MI2S_SLAVE_MODE
	#if defined (CONFIG_TARGET_PRODUCT_IF_S1200N)
		#define USE_MIC_TYPE_DMIC
	#endif
#endif

#ifdef USE_QUAT_MI2S_SLAVE_MODE
	#ifdef USE_MIC_TYPE_DMIC
		#define QUAT_DAI_CLK_MODE SND_SOC_DAIFMT_CBM_CFM
	#else
		#define QUAT_DAI_CLK_MODE SND_SOC_DAIFMT_CBM_CFM | SND_SOC_DAIFMT_LEFT_J
	#endif
	
#else
	#define QUAT_DAI_CLK_MODE SND_SOC_DAIFMT_CBM_CFS
#endif
...
static void *def_tasha_mbhc_cal(void);
	...
	extern int tas2559_cdc_mclk_enable(struct snd_soc_codec *codec, int enable, bool dapm);
	extern int tas2560_cdc_mclk_enable(struct snd_soc_codec *codec, int enable, bool dapm);
	extern void tas5766_powerdown_request(bool enable);
	extern void tas2560_powerdown_request(bool enable);
...
#if defined(CONFIG_SND_SOC_CX2092X)
	static int tx_bits_per_sample = 32;
#endif
...
#if defined(CONFIG_SND_SOC_TAS2559)
static inline bool is_tas5766_snd_card(struct snd_soc_card *card)
{
	if(	strcmp(card->name, "msm8x09-tas5766-snd-card") == 0)
		return true;
	return false;
}

static inline bool is_tas2560_snd_card(struct snd_soc_card *card)
{
	if(	strcmp(card->name, "msm8x09-tas2560-snd-card") == 0)
		return true;
	return false;
}
#endif
...
{
#if defined(CONFIG_SND_SOC_CX2092X)
	return mi2s_tx_sample_rate * tx_bits_per_sample * 2;
#else
...
static int msm_q6_enable_mi2s_clocks(struct snd_pcm_substream *substream, bool enable)
{
	...
	if (enable) 
	{
		...
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			port_config.i2s.channel_mode = MI2S_DATAOUT_PORT;
			if(msm_pri_mi2s_rx_ch >= 2)	
					...
					int enable,	bool dapm)
{
	...
	pr_debug("%s: tas2559_cdc_mclk_enable()", __func__);
	
	if(is_tas2560_snd_card(card)) {
		if(!enable)
			return 0;		
		tas2560_cdc_mclk_enable(codec, enable, dapm);
	} else {
		tas2559_cdc_mclk_enable(codec, enable, dapm);
	}
...
#elif defined(CONFIG_SND_SOC_RT5640)
	pr_debug("%s: rt5640_cdc_mclk_enable()", __func__);
    rt5640_cdc_mclk_enable(codec, enable, dapm);
#else
...
static void msm_mi2s_snd_shutdown(struct snd_pcm_substream *substream)
{
	...
	if (!pdata->codec_type) {
		...
		#if defined(CONFIG_SND_SOC_TAS2559)		
		if(is_tas5766_snd_card(card))
			tas5766_powerdown_request(true);
		else if(is_tas2560_snd_card(card))
			tas2560_powerdown_request(true);
		#endif
...
static int msm_quat_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if (!pdata->codec_type &&
		...
		#ifndef USE_QUAT_MI2S_SLAVE_MODE
		ret = ext_mi2s_clk_ctl(substream, true);
		if (ret < 0) {
			pr_err("%s: failed to enable sclk\n", __func__);
			return ret;
	...
	if (atomic_inc_return(&quat_mi2s_clk_ref) == 1) {
		ret = snd_soc_dai_set_fmt(cpu_dai, QUAT_DAI_CLK_MODE);
		if (ret < 0)
			pr_debug("%s: set fmt cpu dai failed\n", __func__);	
	}
	return ret;
...
static int msm_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if(first_clock == false)
	{
		...
		if(is_tas5766_snd_card(card))
			tas5766_powerdown_request(false);
		else if(is_tas2560_snd_card(card))
			tas2560_powerdown_request(false);
	}
...
static int msm_audrx_init_tas2559(struct snd_soc_pcm_runtime *rtd)
{
	...
	snd_soc_dapm_new_controls(dapm, tas2559_dapm_widgets,
                ARRAY_SIZE(tas2559_dapm_widgets));
...
#if defined(CONFIG_TARGET_PRODUCT_IF_S300N) || defined(CONFIG_TARGET_PRODUCT_IF_S600N) || defined(CONFIG_TARGET_PRODUCT_IF_S600NL) || defined(CONFIG_TARGET_PRODUCT_IF_S1200N)
	{/* hw:x,29 */
		.name = "MSM8X16 Media8",
			...
			struct msm8916_asoc_mach_data *pdata)
{
	...
	if (pdata->us_euro_gpio < 0) {
		...
		if (ret < 0) {
			pr_err("[%s] failed to configure the gpio\n", __func__);
			return ret;
		}
...
#if defined(CONFIG_SND_SOC_TAS2559) || defined(CONFIG_SND_SOC_RT5640)
	if (pdev->id >= 1 && pdev->id < MAX_SND_CARDS) {    
#else
	if (pdev->id == 1) {
#endif
